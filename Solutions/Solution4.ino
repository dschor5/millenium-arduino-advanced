...
void loop()
{
  ...
  switch(state)
  {
    ...
    case STATE_TURN_SIMON:
    {
      ...
      // Iterate thorugh all the keys for that level
      // and print the sequence with short delays in between.
      for(count = 0; count < level; count++)
      {
        updateLcdForLevel(count+1, sequence[count]);
        delay(1500);
      }
    ...
    case STATE_TURN_USER:
    {
      ...
      // Iterate through all the keys the user is
      // expected to press.
      for(count = 0; count < level; count++)
      {
        // Read a key and update the display to show
        // what the user just pressed.
        keyPressed = readKey();
        updateLcdForLevel(count+1, keyPressed);

        // If the key does not match the expected 
        // value, then you lost. 
        if(keyPressed != sequence[count])
        {
          state = STATE_LOSER;
          break;
        }
        // Else if it matches, check if it is the last
        // key press for that level. If so, increase the
        // level and go to the next state.
        // Otherwise, it just loops to get the next key press.
        else if(count+1 == level)
        {
          level++;
          state = STATE_TURN_SIMON;

          // If you've reached the last level, then 
          // you won! Go out an celebrate!!!
